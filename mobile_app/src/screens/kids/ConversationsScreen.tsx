  ...
  return (
    <Screen>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable style={styles.row} onPress={() => nav.navigate('Chat', { peerId: item.peer_id })}>
            <Avatar uri={item.peer_avatar_url} name={item.peer_name} size={48} />
            <Text style={styles.name}>{item.peer_name ?? 'Friend'}</Text>
            <Text style={styles.last}>{item.last_message?.message_text ?? ''}</Text>
