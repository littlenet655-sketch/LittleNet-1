      ...
      floatingActionButton: index == 0
          ? FloatingActionButton.small(
              ...
              onPressed: () => Navigator.of(context).push(
                MaterialPageRoute(
                    builder: (_) => MessagesPage(api: widget.api)),
              ),
...
class _DiscoverPageState extends State<DiscoverPage> {
  ...
  void _load() => setState(() {
        future = widget.api.getJson('/api/mobile/v1/kids/discover',
                    ...
                    children: [
                      if (data['pii_warning'] == true)
                        const Card(
                          child: ListTile(
                            ...
                            subtitle: Text(
                                'Phone numbers, addresses and private contact details cannot be searched.'),
                          ),
                        ),
                      const Padding(
                      ...
                      if (kids.isEmpty)
                        const Padding(
                            ...
                            child: Text(
                                'No children are available in your approved network.')),
                      ...kids.map((kid) => _PersonCard(
                          api: widget.api, kid: kid, onChanged: _load)),
                      const Padding(
                        padding: EdgeInsets.fromLTRB(6, 18, 6, 8),
                        child: Text('Safe posts',
                            style: TextStyle(
                                fontSize: 19, fontWeight: FontWeight.w900)),
                      ),
                      ...posts.map((post) => PostCard(
                          api: widget.api, post: post, onChanged: _load)),
                    ],
                  ),
                );
              },
...
class _PersonCard extends StatelessWidget {
  const _PersonCard(
      {required this.api, required this.kid, required this.onChanged});
  final ApiClient api;
  ...
  Future<void> _toggle(BuildContext context) async {
    try {
      await api
          .postJson('/api/mobile/v1/kids/follow/${kid['user_id']}', const {});
      onChanged();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Card(
      child: ListTile(
        ...
        leading:
            Avatar(api: api, url: kid['avatar_url']?.toString(), radius: 24),
        title: Text(kid['full_name']?.toString() ?? 'LittleNet friend',
            style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle: Text(
            kid['recommendation_reason']?.toString() ?? 'Approved network'),
        trailing: FilledButton.tonal(
          onPressed: () => _toggle(context),
          child: Text(kid['is_following'] == true
              ? 'Friends'
              : kid['is_pending'] == true
                  ? 'Pending'
                  : 'Connect'),
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      toast(
          context,
          status == 'REVIEW'
              ? 'Uploaded and waiting for safety review.'
              : 'Published safely!');
      caption.clear();
      setState(() => media = null);
      widget.onCreated();
        ...
        children: [
          ...
          TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Caption',
                  hintText: 'Share something kind, creative or educational…')),
          ...
          DropdownButtonFormField<String>(
            ...
            items: const ['ALL', '6-8', '9-11', '12-13', '14-18']
                .map((v) => DropdownMenuItem(
                    value: v, child: Text(v == 'ALL' ? 'All allowed ages' : v)))
                .toList(),
          ...
          const Text(
              'LittleNet checks text, photos and video before other children can see them.',
...
class _MessagesPageState extends State<MessagesPage> {
  ...
  void _load() => setState(
      () => future = widget.api.getJson('/api/mobile/v1/kids/messages'));
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (rows.isEmpty) {
            return const Center(
                child:
                    Text('Connect with an approved friend to start chatting.'));
          }
          return RefreshIndicator(
            onRefresh: () async => _load(),
            child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return ListTile(
                  ...
                  leading: Avatar(
                      api: widget.api,
                      url: row['peer_avatar_url']?.toString(),
                      radius: 25),
                  title: Text(row['peer_name']?.toString() ?? 'Friend',
                      style: const TextStyle(fontWeight: FontWeight.w800)),
                  subtitle: Text(last['message_text']?.toString() ??
                  ...
                  onTap: () async {
                    await Navigator.of(context).push(MaterialPageRoute(
                        builder: (_) => ChatPage(
                            api: widget.api,
                            peerId: asInt(row['peer_id']),
                            peerName:
                                row['peer_name']?.toString() ?? 'Friend')));
                    _load();
                  },
                );
              },
...
class _ChatPageState extends State<ChatPage> {
  ...
  void _load() => setState(() =>
      future = widget.api.getJson('/api/mobile/v1/kids/chat/${widget.peerId}'));
  ...
  Future<void> _send() async {
    ...
    try {
      final result = await widget.api.postJson(
          '/api/mobile/v1/kids/chat/${widget.peerId}', {'message_text': text});
      input.clear();
      if (mounted && result['status'] == 'REVIEW') {
        toast(context, 'Message is checking for safety.');
      }
      _load();
    } catch (e) {
      if (mounted) toast(context, friendlyError(e));
                ...
                return RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, reverseIndex) {
                      ...
                      final mine =
                          asInt(item['receiver_child_id']) == widget.peerId;
                                ...
                                Text(text,
                                    style: TextStyle(
                                        color: mine
                                            ? Colors.white
                                            : Colors.black87)),
                              if (item['moderation_status'] == 'REVIEW')
                                Padding(
                                    padding: const EdgeInsets.only(top: 5),
                                    child: Text('Checking for safety…',
                                        style: TextStyle(
                                            fontSize: 11,
                                            color: mine
                                                ? Colors.white70
                ...
                children: [
                  Expanded(
                      child: TextField(
                          ...
                          onSubmitted: (_) => _send(),
                          decoration: const InputDecoration(
                              counterText: '', hintText: 'Message safely…'))),
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void _load() => setState(
      () => future = widget.api.getJson('/api/mobile/v1/kids/profile'));
  ...
  Future<void> _edit(Map<String, dynamic> profile) async {
    ...
    final school =
        TextEditingController(text: profile['school_name']?.toString() ?? '');
    final klass =
        TextEditingController(text: profile['current_class']?.toString() ?? '');
    ...
    for (final c in [name, bio, school, klass]) {
      c.dispose();
    }
    if (data == null) return;
    try {
          ...
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                ...posts.map((post) =>
                    PostCard(api: widget.api, post: post, onChanged: _load)),
              ],
            ),
          );
        },
